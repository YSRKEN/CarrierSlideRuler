	...
	class MipProblem : IDisposable {
		...
		public string ToLpString() {
			...
			for(int ci = 0; ci < ColumnsCount; ++ci) {
				...
				//output += $" {SignedToString(ObjCoef[ci])} {ColumnName[ci]}";
			}
			output += "\n";
			// 制約式
			output += "subject to\n";
			for(int ri = 0; ri < RowsCount; ++ri) {
				...
				foreach(var pair in sortedMatrix) {
					...
					//equationLeft += $" {SignedToString(pair.Value)} {ColumnName[pair.Key]}";
				}
			...
			output += "general\n";
			for(int ri = 0; ri < ColumnsCount; ++ri) {
				if((int)ColumnKind[ri] == (int)VariableKind.Integer) {
					output += $" x{ri + 1}";
				}
			}
			output += "\nbinary\n";
			for(int ri = 0; ri < ColumnsCount; ++ri) {
				if((int)ColumnKind[ri] == (int)VariableKind.Binary) {
					output += $" x{ri + 1}";
				}
			}
			output += "\n";
